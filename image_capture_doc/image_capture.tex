...
\subsubsection{Procedure}
To test for ISO invariance, we will shoot a scene that is very underexposed (ie -6 stops) at the camera's lowest ISO. The ISO is then raised in one-stop increments without changing any other exposure settings and a clip is shot for each of these ISOs. In post, we will match the exposures and compare noise in the shadow regions. Make sure to include a shot of each base ISO too as additional exposures, in case those were skipped by the one-stop increments.
...
\begin{enumerate}
    ...
    \item \textbf{Dark shadows} - The scene should have a range of tones, particularly including those that reach into the shadows. Highlights are appreciated too, so that the clipping behavior can be compared between ISOs.
\end{enumerate}
