...
def fit_bracketed_exposures(args):
    ...
    files = [
        ...
        or x.lower().endswith("tiff")
        or x.lower().endswith("exr")
    ]
    all_images = []
    ...
    if args.blur_amt > 0:
        all_images = [
            cv2.blur(img, (args.blur_amt, args.blur_amt)) for img in all_images
        ]
    ...
    white_pixels_per_image = np.count_nonzero(
        flattened_images >= 0.95 * white_point, axis=1
    )
    brightest_picture_idx = np.argmax(white_pixels_per_image)
    print(
        f"Picture with the most clipped pixels (> 0.95 * {white_point}): {files[brightest_picture_idx]}"
    )
    ...
    print(
        f"Picture with the black pixels (black point: {darkest_value}): {files[darkest_picture_idx]}"
    )

    # Remove the bright and dark image from the dataset.
    all_images = np.delete(
        all_images, [brightest_picture_idx, darkest_picture_idx], axis=0
    )
    ...
    if args.exposures is not None:
        default_exposure_comp = np.array(
            sorted([float(x) for x in args.exposures.split(",")], reverse=True)
        )
        median_image_idx = list(default_exposure_comp).index(0)
        assert (
            default_exposure_comp.shape[0] == all_images.shape[0]
        ), "Incorrect number of exposures specified with --exposures."
    ...
    sampled_coords = np.random.choice(input_images.shape[1], 1000), np.random.choice(
        input_images.shape[2], 1000
    )
    sampled_input = np.mean(
        input_images[median_image_idx, sampled_coords[0], sampled_coords[1], :], axis=1
    )
    for i in range(output_images.shape[0]):
        sampled_output = np.mean(
            output_images[i, sampled_coords[0], sampled_coords[1], :], axis=1
        )  # (1000, 3)
        plt.scatter(
            sampled_input,
            sampled_output,
    ...
    plt.plot(
        [np.min(sampled_input), np.max(sampled_input)],
        [np.min(sampled_input), np.max(sampled_input)],
        color="black",
    )
...
def fit_two_images(args):
    ...
    with torch.no_grad():
        ...
        flattened_reconstructed_lin_image = np.reshape(
            reconstructed_lin_image, (h * w, c)
        )
        reconstructed_log_image = (
            model.reverse(torch.tensor(lin_image)).detach().numpy()
        )
    ...
    for i, color in enumerate(["red", "green", "blue"]):
        plt.scatter(
            flattened_log_image[sampled_coords, i],
            flattened_reconstructed_log_image[sampled_coords, i],
            alpha=0.05,
            marker=".",
            color=color,
            edgecolors=None,
    ...
    for i, color in enumerate(["red", "green", "blue"]):
        plt.scatter(
            flattened_lin_image[sampled_coords, i],
            flattened_reconstructed_lin_image[sampled_coords, i],
            alpha=0.05,
            marker=".",
            color=color,
            edgecolors=None,
